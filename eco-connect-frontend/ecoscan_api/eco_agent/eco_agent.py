...
async def describeProducts(userMedicalAilments: str, file: UploadFile = File(...)):
    ...
    video_path = os.path.join('media', 'videos', 'sunnyD.mp4')
    with open(video_path, 'wb') as jammer:
        jammer.write(file_content)

    product_details = eval(typeDocInputNOutputFormat(model, product_description_template, EdibleDataExtraction, video_path))
